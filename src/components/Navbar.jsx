...
import college from "../assets/img/nav/college.svg";
import Graduate from "../assets/img/nav/Graduate.svg";
import MBA from "../assets/img/nav/MBA.svg";
import Medical from "../assets/img/nav/Medical.svg";

const Navbar = () => {
 const [activeProgram, setActiveProgram] = useState(null); // Changed to null initially
  ...
  const location = useLocation();
    ...
    if (location.pathname === "/") {
      setActiveProgram(null); // No active program on home page
    } else {
      const currentProgram = programs.findIndex(
        (program) => program.href === location.pathname
      );
      setActiveProgram(currentProgram !== -1 ? currentProgram : null);
    }
  }, [location.pathname]);

  const programs = [
    { label: "College", href: "/college", img:college },
    { label: "Graduate", href: "/graduate", img:MBA },
    { label: "MBA", href: "/mba" , img:Graduate},
    { label: "Medical", href: "/medical", img:Medical },
  ];
          ...
          <div className="flex overflow-x-auto scrollbar-hide">
            {programs.map((program, index) => (
              ...
              >
                ...
                >
                  <img src={program.img} className="w-4 h-4 mr-2" alt="" />
                  {program.label}
                </Link>
                {activeProgram === index && (
                  <motion.div
